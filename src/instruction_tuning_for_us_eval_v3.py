...
# Training Settings
TRAIN_BATCH_SIZE = 2          # Number of samples per training batch (reduce if out of memory)
EVAL_BATCH_SIZE = 8            # Number of samples per validation batch
LEARNING_RATE = 3e-5           # How fast the model learns (lower = more stable)
NUM_EPOCHS = 5                # How many times to go through the entire dataset
...
def test_multitask_model(model, tokenizer, device, test_samples=None):
    ...
    for task_name, metrics in task_summaries.items():
        ...
        if "accuracy" in metrics:
            ...
            if task_name == "code_repair":
                print(f"    BLEU: {metrics['bleu']:.3f}")   # ← 추가됨
